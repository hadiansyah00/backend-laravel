    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900" x-data="permissionForm()" x-init="$refs.input.focus()">

                    <!-- Form untuk menambah permission ke daftar -->
                    <div class="flex items-center mb-4 space-x-2">
                        ...
                        <button @click="addPermission"
                            class="px-4 py-2 font-bold text-white bg-indigo-500 rounded hover:bg-indigo-700 whitespace-nowrap"
                            :disabled="loading">
                            + Tambah
                        </button>
                    </div>
                    ...
                    <template x-if="message">
                        <div x-text="message" :class="message.startsWith('Error') ? 'text-red-600' : 'text-green-600'"
                            class="mb-4 font-bold"></div>
                    </template>
                    <template x-if="errors['permissions.0.name']">
                        <div class="mb-4 text-sm text-red-600" x-text="'Error: ' + errors['permissions.0.name'][0]">
                        </div>
                    </template>

                    <!-- Daftar permission yang akan disimpan -->
                    <h3 class="mb-2 font-bold">Daftar Permission untuk Disimpan:</h3>
                    <div class="border rounded-md p-4 min-h-[100px] bg-gray-50">
                        <template x-if="permissions.length === 0">
                            <p class="text-center text-gray-500">Belum ada permission yang ditambahkan.</p>
                        </template>
                        <ul class="space-y-2">
                            <template x-for="(permission, index) in permissions" :key="permission.name">
                                <li class="flex items-center justify-between p-2 bg-white border rounded-md shadow-sm">
                                    <span x-text="permission.name" class="text-gray-800"></span>
                                    <button @click="removePermission(index)"
                                        class="font-bold text-red-500 hover:text-red-700" :disabled="loading">
                                        &times;
                                    </button>
                                </li>
                            </template>
                    ...
                    <div class="flex items-center justify-end mt-6">
                        ...
                        <button @click="savePermissions" :disabled="loading || permissions.length === 0"
                            class="flex items-center px-4 py-2 font-bold text-white bg-blue-500 rounded hover:bg-blue-700 focus:outline-none focus:shadow-outline"
        ...
        function permissionForm() {
            return {
                ...
                async savePermissions() {
                    ...
                    try {
                        const response = await fetch('{{ route('permissions.storeMultiple') }}', {
                            method: 'POST',
                            headers: {
